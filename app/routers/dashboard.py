...
def get_dashboard_data(email: str, db: Session = Depends(get_db)):
    ...
    # 3. Retrieve Synced Health Data
    health_data = crud.get_user_health_data(db, email)
    
    # Establish baseline values
    steps = health_data.steps if health_data else 0
    calories = health_data.calories if health_data else 0
    sleep = health_data.sleep_duration_hours if health_data else 0.0
    water = health_data.water_intake_ml if health_data else 0
    workouts = health_data.workouts_count if health_data else 0
    heart_rate = health_data.heart_rate_bpm if health_data else 72
    ...
    steps_score = min(steps / step_target, 1.0) * 30
    calories_score = min(calories / calorie_target, 1.0) * 25
    sleep_score = min(sleep / sleep_target, 1.0) * 25
    water_score = min(water / water_target, 1.0) * 20
